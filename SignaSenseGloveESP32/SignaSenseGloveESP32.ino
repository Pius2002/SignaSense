/*
  ...
  - Creates a local Wi-Fi network for the SignaSense phone app.
  - Hosts a local page at http://192.168.4.1 with live captions and audio.
  - No internet or mobile data is required after the phone joins this Wi-Fi.

  Phone use:
  1. Power the ESP32 glove.
  2. Connect the phone to Wi-Fi network: SmartGlove
  3. Password: 12345678
  4. Open SignaSense, choose Smart Glove, or open http://192.168.4.1
  ...
  Important ESP32 ADC limitation:
  - GPIO25 is ADC2.
  - On the classic ESP32, ADC2 readings are unreliable while Wi-Fi is active.
  - This code keeps Thumb on GPIO25 because that is the requested wiring, but
    it treats the thumb as optional for recognition if Wi-Fi makes it unreadable.
  - The other four pins, GPIO32 to GPIO35, are ADC1 and should read during Wi-Fi.
...
void setup() {
  ...
  Serial.println();
  Serial.println("SignaSense ESP32 Smart Glove starting...");
  Serial.println("Pin map: Thumb=GPIO25, Index=GPIO33, Middle=GPIO32, Ring=GPIO35, Pinky=GPIO34");
  Serial.println("Thumb GPIO25 is ADC2 and may not read while Wi-Fi is active.");
  Serial.print("Connect phone to Wi-Fi: ");
  Serial.println(WIFI_AP_NAME);
  Serial.print("Password: ");
  Serial.println(WIFI_AP_PASSWORD);
  Serial.println("Open app glove page or browser: http://192.168.4.1");
}
...
void handleData() {
  ...
  json += "\"ok\":true";
  json += ",\"device\":\"SignaSense Smart Glove\"";
  json += ",\"ms\":";
  json += millis();
...
String statusText() {
  ...
  if (active == 0) {
    return "No ADC1 sensor signal yet. Check 3.3V, GND, divider resistors, and GPIO32 to GPIO35 wiring.";
  }

  if (active < required) {
    return "Partial sensor signal. The app is reading what is available; check any finger marked no signal.";
  }

  if (!sensorHasSignal[FINGER_THUMB]) {
    return "Core fingers ready. Thumb on GPIO25 may be blocked by Wi-Fi, so recognition continues without it.";
  }
...
String tipText() {
  if (!sensorHasSignal[FINGER_THUMB]) {
    return "Thumb is wired to GPIO25. That pin is ADC2, so it may show no signal while Wi-Fi is active.";
  }
